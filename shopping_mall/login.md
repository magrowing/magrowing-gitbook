...
- 쿠키를 호함한 요청이 외부에 노출되어도 세션ID 자체는 유의미한 개인 정보를 담지 않는다.
...
2. 서버는 `비밀키`를 사용해 Json 객체를 암호화한 `JWT 토큰(Access Token)발급`
3. Access Token을 헤더에 담아 클라이언트에 보낸다.

여기까지가 JWT(Access Token)을 발급받기까지의 과정, 로그인 이후에는

1. 클라이언트는 JWT(Access Token)을 `브라우저 저장소` 저장해둔다.
